...

{
  ...
  do
    ...
    java --add-opens=java.base/java.util=ALL-UNNAMED --add-opens=java.base/sun.util.calendar=ALL-UNNAMED --add-opens=java.base/java.lang=ALL-UNNAMED \
     ...
     -Dsearch_budget=900 -Dminimize=true -Dcoverage=true -Dwrite_junit_timeout=100000 -Dextra_timeout=10000 \
     ...
     -Dcheck_contracts=false -Dllm_static_constant_pool=false -Dsandbox=false -Dvirtual_fs=false -Dvirtual_net=false -Duse_separate_classloader=false -Dno_runtime_dependency=false -Dreset_static_fields=false \
